...
class FactCheckJob < ApplicationJob
  ...
  def build_judgment_prompt(search_term, internal_info_json, external_results_json)
    ...
    prompt += "| :--- | :--- | :--- | :--- |\n"
    prompt += "| **5** | **非常に高い** | 非常に低い（0-10%） | 非常に高い（90-100%） | 信頼できる情報源の**全て**が、検索内容の**真実性**を一貫して強く支持している。誤情報を示す要素は**皆無**。|\n"
    prompt += "| **4** | **高い** | 低い（11-30%） | 高い（70-89%） | 信頼できる情報源の**大半**が**真実性**を支持。一部に**軽微な疑問点**や**不確かな点**の示唆がある程度。|\n"
    prompt += "| **3** | **真偽不明** | 中程度（31-70%） | 中程度（30-69%） | 真偽について**意見が分かれている**（情報源が矛盾している）、または**情報量が少なく**断定的な判断が難しい。|\n"
    prompt += "| **2** | **低い** | 高い（71-89%） | 低い（11-30%） | 信頼できる情報源、特にファクトチェック記事や公的情報が、検索内容を**偽・誤情報**であると**複数指摘**している。|\n"
    prompt += "| **1** | **非常に低い** | 非常に高い（90-100%） | 非常に低い（0-10%） | 信頼できる情報源の**全て**、特に公的機関やファクトチェックが、検索内容を**偽・誤情報**であると**一貫して強く**指摘している。|\n\n"
